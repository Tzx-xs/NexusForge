...
/// 后端可执行文件名（Windows 有 .exe 后缀，Linux/macOS 无后缀）
#[cfg(target_os = "windows")]
const BACKEND_EXE: &str = "nexusforge-backend.exe";
#[cfg(not(target_os = "windows"))]
const BACKEND_EXE: &str = "nexusforge-backend";
/// 后端目录名
const BACKEND_DIR: &str = "nexusforge-backend";
...
impl BackendManager {
    ...
    fn inject_prod_data_env(cmd: &mut Command, handle: &AppHandle) -> Result<(), String> {
        ...
        let log_file = logs_dir.join("nexusforge.log");
        cmd.env("LOG_FILE", log_file.as_os_str());

        Ok(())
    }

    /// PyInstaller onedir：`$RESOURCE/{BACKEND_DIR}/{BACKEND_EXE}`（见 tauri.conf resources 映射）
    fn find_frozen_backend_exe(handle: &AppHandle) -> Option<PathBuf> {
        ...
        if let Ok(p) = handle.path().resolve(
            &format!("{}/{}", BACKEND_DIR, BACKEND_EXE),
        ...
        if let Ok(p) = handle.path().resolve(
            &format!("../../out/tauri/{}/{}", BACKEND_DIR, BACKEND_EXE),
            BaseDirectory::Resource,
        ) {
            if p.is_file() {
        ...
        if let Ok(rd) = handle.path().resource_dir() {
            let nested = rd.join(BACKEND_DIR).join(BACKEND_EXE);
            if nested.is_file() {
                log::info!("📦 后端路径 (resource_dir nested): {}", nested.display());
                return Some(nested);
            }
            let flat = rd.join(BACKEND_EXE);
            if flat.is_file() {
            ...
            .and_then(|p| p.canonicalize().ok())
        {
            ...
            // 方案 3：与主 exe 同目录下的 backend/（便携解压布局）
    ...
    pub fn spawn_only(&mut self) -> Result<u16, String> {
        ...
        let mut cmd = if let Some(ref exe) = frozen {
            ...
            let python = self.find_python().ok_or_else(|| {
                "未找到 nexusforge-backend 可执行文件，也未找到 Python。发布构建请运行 PyInstaller 打包脚本；开发请安装 Python 3.14".to_string()
            })?;
