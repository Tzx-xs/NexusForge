<template>
  <div class="work-area">
    <header class="work-header">
      ...
      <div class="work-header-actions">
        ...
        <n-tooltip trigger="hover">
          <template #trigger>
            <n-button size="small" secondary @click="showAgentDrawer = true">
              AI 助手
            </n-button>
          </template>
          与 Agent 对话：生成、审查、查询设定
        </n-tooltip>
      </div>
    </header>
            ...
            <div class="work-main primary-desk-root">
              ...
              >
                ...
                <div class="editor-body">
                  ...
                  >
                    <TiptapEditor
                      v-if="featureFlags.tiptapEditor"
                      :content="editorDisplayContentForTiptap"
                      :readonly="true"
                      @update="handleTiptapUpdate"
                      @word-count="handleTiptapWordCount"
                    />
                    <n-input
                      v-else
    ...
    <!-- Phase 4 Task 4.2：Agent 聊天面板抽屉（右侧滑出） -->
    <n-drawer v-model:show="showAgentDrawer" :width="420" placement="right">
      <n-drawer-content title="AI 助手" closable>
        <AgentChatPanel :novel-id="slug" />
      </n-drawer-content>
    </n-drawer>
...
// Phase 4 Task 4.2：Agent 聊天面板（异步加载，避免首屏包体膨胀）
const AgentChatPanel = defineAsyncComponent(() => import('../agent/AgentChatPanel.vue'))
...
/**
 * TiptapEditor 需要的 HTML 内容。
 * 编辑器内部是 HTML，而 chapterContent 是纯文本（流式输出）。
 * 将纯文本按换行分段转成 <p> 标签；若已是 HTML（含 < 标签）则原样传入。
 */
const editorDisplayContentForTiptap = computed(() => {
  const raw = editorDisplayContent.value
  if (!raw) return '<p class="paragraph"></p>'
  // 已含 HTML 标签则原样返回（避免双重包裹）
  if (/<[a-z][\s\S]*>/i.test(raw)) return raw
  // 纯文本按换行分段
  const paragraphs = raw.split(/\n+/).filter(Boolean)
  if (paragraphs.length === 0) return '<p class="paragraph"></p>'
  return paragraphs.map(p => `<p class="paragraph">${p}</p>`).join('')
})

/** TiptapEditor 内容更新回调（防抖 800ms 后触发） */
const handleTiptapUpdate = (html: string) => {
  // 将 HTML 转回纯文本存储（与原 n-input 行为一致）
  const tempDiv = document.createElement('div')
  tempDiv.innerHTML = html
  chapterContent.value = tempDiv.textContent || tempDiv.innerText || ''
}

/** TiptapEditor 字数变化回调 */
const tiptapWordCount = ref(0)
const handleTiptapWordCount = (count: number) => {
  tiptapWordCount.value = count
}
...
/* Phase 4 Task 4.2：头部右侧操作区（模式开关 + AI 助手按钮） */
.work-header-actions {
  display: flex;
  align-items: center;
  gap: 12px;
