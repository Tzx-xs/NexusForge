  ...
  <div class="agent-chat-panel">
    ...
    <div ref="messagesContainer" class="messages-container">
      <div v-if="messages.length === 0" class="empty-state">
        <p class="empty-title">你好，我是星渊笔 AI 创作助手</p>
        ...
        case 'tool_result': {
          ...
          if (assistant.toolCalls) {
            ...
            if (toolCallId) {
              ...
              // Fallback: match by tool name when tool_call_id is not provided
...
async function toggleHistory() {
  ...
  if (showHistory.value) {
    try {
      ...
      toast.error(err?.message || '加载历史会话失败')
      conversations.value = []
    }
  }
}
...
.tool-call-card {
  background: var(--xy-surface-3, var(--xy-surface-1));
  border: 1px solid var(--xy-border-1);
  border-radius: 6px;
  padding: 8px 10px;
  font-size: 11px;
...
.input-textarea {
  ...
  color: var(--xy-text-1);
  font-size: 13px;
  font-family: var(--xy-font-sans, inherit);
  line-height: 1.5;
  outline: none;
...
.stop-btn {
  background: var(--xy-danger, #ef4444);
  color: #fff;
}
